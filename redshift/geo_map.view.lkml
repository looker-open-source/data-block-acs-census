...
view: logrecno_bg_map_core {
    ...
    derived_table: {
      ...
      FROM
        acs.geo2015 as geo
      LEFT JOIN acs.block_group_attribs as bg on (SUBSTRING(geo.GEOID, 8, 12) = bg.geoid AND geo.SUMLEVEL = '150')
      LEFT JOIN acs.block_group_attribs as tr on (SUBSTRING(geo.GEOID, 8, 11) = SUBSTRING(tr.geoid, 1, 11) AND geo.SUMLEVEL = '140')
      WHERE
        sumlevel in ('140','150')
